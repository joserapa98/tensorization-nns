...
def accs_by_class(sex, indian):
    ...
    for p_indian in p_indian_list:
        ...
        for idx in range(10):
            ...
            if os.path.exists(models_dir):
                ...
                for s in range(n_models):
                    print(f'\t\t{s=}')
                    # Load state_dict
                    cores_dir = list(filter(lambda f: f.startswith(f'{s}_'),
                                                 os.listdir(models_dir)))
                    if cores_dir:
                        cores = torch.load(
                            os.path.join(models_dir, cores_dir[0]),
                            weights_only=False)
                        ...
                        tn_model.trace(
                            torch.zeros(1, 500, 2).to(device),
                            inline_input=False,
                            inline_mats=False
                        )
                        
                        # Test
                        test_accs = test_tn(device, tn_model, embedding,
                                            test_loader, 10)
                        print(f'\t\t\t{test_accs}')
        ...
        aux_dir = os.path.join(cwd, 'results', '6_privacy', model_class.name,
                               'attacks', 'acc_by_class', 'priv_recores', str(p_indian))
