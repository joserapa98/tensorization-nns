...
def accs_by_class(p_indian, sex, indian):
    ...
    for idx in range(10):
        ...
        for s in range(n_models):
            ...
            # Test
            test_accs = test(device, model, test_loader, 10)
            print(f'\t\t\t{test_accs}')
    ...
    # accs.append((p, (p_accs.mean(), p_accs.std())))
    
    aux_dir = os.path.join(cwd, 'results', '6_privacy', model_class.name,
                           'attacks', 'acc_by_class', str(p_indian))
    os.makedirs(aux_dir, exist_ok=True)
    
    torch.save(p_accs,
               os.path.join(aux_dir, f'{sex}_{indian}.pt'))
...
if __name__ == '__main__':
    ...
    accs_by_class(p_indian, sex, indian)
