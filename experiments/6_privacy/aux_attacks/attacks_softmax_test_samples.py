...
def create_dataset(p_indian, idx):
    ...
    unused_loader_iter = iter(unused_loader)
    ...
    for samples, (labels, p_targets) in unused_loader_iter:
        print(samples.size(0))
        for i, (l, p) in enumerate(combs):
            aux_samples = samples[(labels == l) * (p_targets == p)]
            all_samples[i].append(aux_samples[:all_n_samples[i]])
            
            all_n_samples[i] -= all_samples[i][-1].size(0)
    ...
    if all_samples.size(0) < (n_samples * 4):
        print(f'Not enough samples of type {combs[i]}')
        assert False
    
    aux_dir = os.path.join(cwd, 'results', '6_privacy', 'fffc_tiny',
                           'attacks_softmax_test_samples', 'attack_samples',
                           str(p_indian))
...
def save_models_data(model_type='nn',
    ...
    all_results_dir = os.path.join(cwd, 'results', '6_privacy',
                                   'fffc_tiny', 'attacks_softmax_test_samples' )
    ...
    for p in p_indian_list:
        ...
        for idx in range(10):
            ...
            # Load attack samples
            samples_dir = os.path.join(all_results_dir, 'attack_samples',
                                       str(p), f'{idx}.pt')
            if not os.path.exists(samples_dir):
                create_dataset(p, idx)
