...
def create_dataset(p_indian, idx):
    # Load data
    batch_size = 1000
    _, unused_loader = load_sketch_samples(p_indian, idx, batch_size)
    
    all_n_samples = [0, 0, 0, 0]
    all_samples = [[], [], [], []]
    combs = [(0, 0), (0, 1), (1, 0), (1, 1)]
    
    for samples, (labels, p_targets) in unused_loader:
    ...
    aux_dir = os.path.join(cwd, 'results', '6_privacy', 'fffc_tiny',
                           'attacks_softmax_same_test_samples', 'attack_samples',
                           str(p_indian))
    os.makedirs(aux_dir, exist_ok=True)
    
    torch.save(all_samples, os.path.join(aux_dir, f'{idx}.pt'))
...
def save_models_data(model_type='nn',
    ...
    all_results_dir = os.path.join(cwd, 'results', '6_privacy',
                                   'fffc_tiny', 'attacks_softmax_same_test_samples' )
    ...
    for idx in range(10):
        # Load attack samples
        samples_3_dir = os.path.join(all_results_dir, 'attack_samples',
                                    '0.3', f'{idx}.pt')
        if not os.path.exists(samples_3_dir):
            create_dataset(0.3, idx)
        
        samples_7_dir = os.path.join(all_results_dir, 'attack_samples',
                                    '0.7', f'{idx}.pt')
        if not os.path.exists(samples_7_dir):
            create_dataset(0.7, idx)
    ...
    for idx in range(10):
        ...
        samples_3_dir = os.path.join(all_results_dir, 'attack_samples',
                                     '0.3', f'{idx}.pt')
        samples_3 = torch.load(samples_3_dir, weights_only=False).to(device)
        
        samples_7_dir = os.path.join(all_results_dir, 'attack_samples',
                                     '0.7', f'{idx}.pt')
        samples_7 = torch.load(samples_7_dir, weights_only=False).to(device)
        
        samples, middle_point = intersection(samples_3, samples_7)
        ...
        for p in [0.3, 0.7]:
            ...
            for s in range(n_models):
                ...
                results_women = results[:middle_point, 0]
                results_men = results[middle_point:, 1]
                ...
                torch.save((results, samples.size(0)),
                           os.path.join(aux_results_dir, f'{s}.pt'))
