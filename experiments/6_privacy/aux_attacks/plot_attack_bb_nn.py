...
for p in p_indian_list:
    ...
    for idx in range(10):
        ...
        models_dir = os.path.join(cwd, 'results', '0_train_nns', 
                                  model_class.name, str(p), str(idx))
        ...
        for s in range(n_models):
            ...
            # Test
            test_acc = test(device, model, test_loader, batch_size, 5)
            
            p_accs.append(test_acc)
            
    p_accs = torch.Tensor(p_accs)
    accs.append((p, (p_accs.mean(), p_accs.std())))

torch.save(accs, os.path.join(cwd, 'results', '6_privacy', model_class.name,
